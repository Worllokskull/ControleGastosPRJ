...
function App() {
  ...
  const [formEdicaoPessoa, setFormEdicaoPessoa] = useState<FormPessoa>({ nome: '', idade: 0 });
  ...
  const selecionarPessoa = async (id: string) => {
    try {
      ...
      setFormEdicaoPessoa({ nome: data.nome, idade: data.idade });
    } catch (error) {
      setMessage((error as Error).message);
    }
  };
  ...
  const atualizarPessoa = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!pessoaSelecionada) return;
    try {
      const response = await fetch(`${API_URL}/Pessoas/${pessoaSelecionada.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formEdicaoPessoa)
      });
      ...
      <main className="grid">
        <section className="card">
          ...
          <form onSubmit={criarPessoa} className="form">
            <input placeholder="Nome" value={formPessoa.nome} onChange={(e) => setFormPessoa({ ...formPessoa, nome: e.target.value })} />
            <input type="number" placeholder="Idade" value={formPessoa.idade} onChange={(e) => setFormPessoa({ ...formPessoa, idade: Number(e.target.value) })} />
        ...
        <section className="card">
          ...
          {pessoaSelecionada ? (
            <>
              ...
              <form onSubmit={atualizarPessoa} className="form">
                <input
                  placeholder="Nome"
                  value={formEdicaoPessoa.nome}
                  onChange={(e) => setFormEdicaoPessoa({ ...formEdicaoPessoa, nome: e.target.value })}
                />
                <input
                  type="number"
                  placeholder="Idade"
                  value={formEdicaoPessoa.idade}
                  onChange={(e) => setFormEdicaoPessoa({ ...formEdicaoPessoa, idade: Number(e.target.value) })}
                />
              ...
              <form onSubmit={criarTransacao} className="form">
                <input placeholder="Descrição" value={formTransacao.descricao} onChange={(e) => setFormTransacao({ ...formTransacao, descricao: e.target.value })} />
                <input type="number" step="0.01" placeholder="Valor" value={formTransacao.valor} onChange={(e) => setFormTransacao({ ...formTransacao, valor: Number(e.target.value) })} />
